...
exit_if_changelog_is_up_to_date () {
  if grep "<!-- kubebuilder ${head_version} -->" CHANGELOG.md; then
...
generate_changelog () {
  ...
  local scaffold_commit_sha="$(git log -1 --grep '(scaffold / project)' --format=%H "${GITHUB_BASE_REF}...${GITHUB_HEAD_REF}")"
